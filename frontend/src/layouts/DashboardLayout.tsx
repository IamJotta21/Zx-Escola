  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-background text-foreground transition-colors duration-200">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {/* Global Support Mode Active Banner */}
        {isSupportMode && (
          <div className="bg-amber-500 dark:bg-amber-600 text-slate-950 px-4 py-2 text-xs font-bold flex flex-col sm:flex-row items-center justify-between gap-2 shadow-md border-b border-amber-600 shrink-0 animate-in slide-in-from-top duration-300">
            <div className="flex items-center gap-2">
              <span className="bg-slate-950 text-amber-400 text-[10px] uppercase font-black px-2 py-0.5 rounded tracking-wider flex items-center gap-1">
                <ShieldAlert className="h-3 w-3" /> Modo Suporte Ativo
              </span>
              <span>
                Você está acessando o painel da escola:{' '}
                <strong className="underline decoration-amber-800">{supportTenant?.name || user?.tenantName || 'Escola Selecionada'}</strong>
              </span>
            </div>
            <button
              onClick={exitSupportMode}
              className="bg-slate-950 hover:bg-slate-900 text-white text-xs font-bold px-3 py-1 rounded-md transition-all flex items-center gap-1.5 border border-amber-400/30 cursor-pointer shrink-0 shadow-sm"
              title="Clique para encerrar a sessão de suporte e retornar ao Painel Global"
            >
              <LogOut className="h-3.5 w-3.5" /> Voltar ao Painel Global SaaS
            </button>
          </div>
        )}
